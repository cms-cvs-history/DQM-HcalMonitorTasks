			 ...
			 DQMStore* dbe, string baseFolder)
  {
	...
	*/
	...
	if (digi.sample(i).adc()>calibs.pedestal(thisCapid)+Nsigma*widths.pedestal(thisCapid))
	  {
	    hist.above_pedestal_temp->Fill(digi.id().ieta(),digi.id().iphi(),20);
...

{
  ...
  for (float eta=etaMin_;eta<etaMax_;eta+=1.)
	...
	continue;
      else if (h.type==3 && fabs(eta)>15) continue;
      ...
      for (float phi=phiMin_;phi<phiMax_;phi+=1.)
	{
	  ...
	  cout <<"eta, phi = "<<eta<<" "<<phi<<"\tieta,iphi = "<<int(eta-etaMin_+1)<<" "<<int(phi-phiMin_+1)<<"  BIN CONTENT = "<<temp<<endl;
